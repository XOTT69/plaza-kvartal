    ...
    const buildingSelectHtml = user.isSuperAdmin && buildings.length > 0 ? `
      <div style="margin-bottom: 12px;">
        <select id="aptBuildingFilter" class="form-input" onchange="onAptBuildingFilter(this.value)" style="font-size: 14px;">
          ...
          ${buildings.map(b => `<option value="${b.id}" ${adminSelectedBuilding === b.id ? 'selected' : ''}>${b.name}</option>`).join('')}
        </select>
      </div>
    ` : '';
...
function showEditApartment(docId, code, isAdmin, name, buildingId, aptNumber) {
  showApartmentForm('Редагувати квартиру', docId, code, isAdmin, name, buildingId, aptNumber);
}

async function showApartmentForm(title, docId, code, isAdmin, name, buildingId, aptNumber) {
